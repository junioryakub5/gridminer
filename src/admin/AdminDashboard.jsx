...
import { Users, Wallet, Clock, TrendingUp, BarChart3, ArrowDownCircle, AlertCircle, Loader2 } from 'lucide-react';
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:20 }}>
        ...
        <div className="admin-card">
          <div className="admin-card-header">
            <span className="admin-card-title">⛏ Recent Transactions</span>
            <button className="admin-btn admin-btn-secondary admin-btn-sm" onClick={() => navigate('/admin/transactions')}>View All</button>
          </div>
          {recent.length === 0 ? (
            <div className="admin-empty"><div className="admin-empty-icon">📭</div><p>No transactions yet</p></div>
          ) : (
              ...
              <tbody>
                {recent.map(tx => (
                  <tr key={tx.id}>
                    ...
                    <td><span style={{ display:'flex', alignItems:'center', gap:5, fontSize:12 }}>{TX_ICONS[tx.type] || '💫'} {tx.type}</span></td>
        ...
        <div className="admin-card">
          <div className="admin-card-header">
            <span className="admin-card-title">👥 Users by Tier</span>
          ...
          {pendingUpgrades.length > 0 && (
            <>
              <div style={{ padding:'0 22px 8px', fontSize:12, fontWeight:700, color:'#b45309' }}>⏳ Pending Approvals</div>
